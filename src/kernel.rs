...
mod vector;
pub use crate::vector::{Vector, UnitVector};
mod sources;
pub use crate::sources::{Source, PencilSource, DiskSource};
mod monte_carlo;
pub use crate::monte_carlo::{MonteCarloSpecification, State, Detector, monte_carlo};

use num::complex::Complex32;
use nvptx_sys::{Float, threadIdx, blockIdx, blockDim};
...
unsafe fn kernel<S: Source + ?Sized>(
    ...
    let phi_dist = core::slice::from_raw_parts_mut(phi_dist.add((gid * len) as usize), len as usize);
    let len = (ndet * ntof) as usize;
    let photon_counter = core::slice::from_raw_parts_mut(photon_counter.add((gid * len) as usize), len as usize);
    ...
    let partial_path = core::slice::from_raw_parts_mut((dyn_mem as *mut f32).add(idx), nmedia as usize);
